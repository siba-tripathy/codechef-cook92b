...
void judge()
{
	...
	rep(0,t)
	{
		cin>>a;q.pb(a);amax=max(a,amax);
	}
	vi arr;
	arr.pb(0);int po=1;
	while(arr[arr.size()-1]<=amax)
	{
		arr.pb(arr[po-1]+po);
		po++;
	}
	...
	{
		vi::iterator it;
		it=upper_bound(arr.begin(),arr.end(),q[i]);
		/*cout<<int(it-arr.begin())+ABS(*it-q[i])<<endl;
		//cout<<int(it-arr.begin()-1)+ABS(*(it-1)-q[i])<<endl;
		cout<<int(it-arr.begin()-1);*/
		cout<<min(int(it-arr.begin())+ABS(*it-q[i]),int(it-arr.begin()-1)+ABS(*(it-1)-q[i]))<<endl;
	}
	return;
}
void gcj()
