...
void judge()
{
	...
	rep(0,n)
	{
		...
		if(arr[a-1]==-1&&arr[b-1]==-1)
			{
				vi s;s.pb(a);s.pb(b);arr[a-1]=fin.size();arr[b-1]=fin.size();
				fin.pb(s);
			}
		else if(arr[a-1]!=-1&&arr[b-1]!=-1)
		{
			vi s;
			vi::iterator it=set_union(fin[arr[a-1]].begin(),fin[arr[a-1]].end(),fin[arr[b-1]].begin(),fin[arr[b-1]].end(),s.begin());
			s.resize(it-s.begin());
			fin[arr[a-1]]=vi(1,0);
			fin[arr[a-1]]=vi(1,0);
			arr[a-1]=arr[b-1]=fin.size();
			fin.pb(s);
		}
		else if(arr[a-1]!=-1)
			{
				fin[arr[a-1]].pb(b);arr[b-1]=arr[a-1];
			}
		else if(arr[b-1]!=-1)
			{
				fin[arr[b-1]].pb(a);arr[a-1]=arr[b-1];
			}
	}
	int maxi=0;
	rep(0,(int)fin.size())
	{
		int as=maxint;
		repe(j,0,(int)fin[i].size())
		{
			as&=fin[i][j];
		}
		maxi=max(maxi,as*(int)fin[i].size());
	}	
	cout<<maxi<<endl;
	return;
}
